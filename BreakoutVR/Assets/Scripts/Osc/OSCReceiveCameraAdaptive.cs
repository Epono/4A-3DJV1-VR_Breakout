...
public class OSCReceiveCameraAdaptive : MonoBehaviour {
    ...
    void SetAssymetricFrustum(Vector3 eye_position) {

        // what does this line do ?

        transform.position = eye_position;

        // compute frustum
        // refer to gen-perspective.pdf
        float minx = object_to_look.renderer.bounds.min.x;
        float miny = object_to_look.renderer.bounds.min.y;
        float maxx = object_to_look.renderer.bounds.max.x;
        float maxy = object_to_look.renderer.bounds.max.y;
